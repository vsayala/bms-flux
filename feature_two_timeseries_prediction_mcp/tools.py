...
def train_timeseries_models(df, target_columns):
    ...
    for target in target_columns:
        ...
        model = XGBRegressor(objective="reg:squarederror", n_estimators=500, learning_rate=0.01, max_depth=5, subsample=0.8, colsample_bytree=0.8, random_state=42)
...
@timing_decorator
def plot_timeseries_predictions(df, predictions, target_columns, cell_id, plots_folder, steps=10):
    ...
    for i, target in enumerate(target_columns):
        ax = axes[i]
        ax.plot(df["Timestamp"], df[target], label=f"Historical {target}", color="blue", alpha=0.7)
        ax.scatter(
...
def run_full_timeseries_pipeline(file_path, cell_id, steps=10):
    ...
    plot_path = plot_timeseries_predictions(df, predictions, target_columns, cell_id, plots_folder, steps)
    return {
        "metrics": metrics,
