...
def clean_and_filter_data(data_path: str, chunksize: int = 100000) -> dict:
    try:
        ...
        schema_path = export_schema(
            df, os.path.join(run_folder, "battery_cleaned_schema.json")
        )
...
def engineer_battery_features(csv_path: str) -> dict:
    try:
        ...
        schema_path = export_schema(
            df, os.path.join(run_folder, "battery_with_features_schema.json")
        )
...
def impute_battery_data_mice(csv_path: str) -> dict:
    try:
        ...
        schema_path = export_schema(
            df, os.path.join(run_folder, "battery_mice_imputed_schema.json")
        )
...
def impute_and_scale_battery_data(csv_path: str) -> dict:
    try:
        ...
        schema_path = export_schema(
            df, os.path.join(run_folder, "battery_final_scaled_schema.json")
        )
...
@mcp.tool()
def preprocess_battery_data_tool(data_path: str, chunksize: int = 100000) -> dict:
    try:
        ...
        out_path = (
            os.path.join(run_folder, "battery_preprocessed.csv") if run_folder else None
        )
        log_path = (
            os.path.join(run_folder, "preprocess-log.log") if run_folder else None
        )
        schema_path = (
            os.path.join(run_folder, "battery_preprocessed_schema.json")
            if run_folder
            else None
        )
